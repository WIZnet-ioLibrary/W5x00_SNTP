...
{
	static uint16_t user0_pressed_check_cnt = 0;
	static  user0_press_detected = false;

	msTicks++; // increment counter

	//user0control
	if(user0_enable == true)
	{
		if(!user0_press_detected)
		{
			user0_pressed_check_cnt++;
			if(user0_pressed_check_cnt >= BUTTONS_PRESSED_TICKS)
			{
				user0_pressed_flag = true;
				user0_pressed_check_cnt = 0;
				user0_enable = false;

				user0_press_detected = true;
			}
		}
	}
	else
